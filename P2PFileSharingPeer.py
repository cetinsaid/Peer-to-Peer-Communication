...
if len(sys.argv) < 5:
    c.Log("Usage: python P2PFileSharingPeer <Server IP>:<Server Port Number> <File Repository> <Schedule File>")
    sys.exit(1)
...
def sendPartialDownloadRequest(conn, file_partial, start_byte, end_byte):
    ...
    try:
        ...
        c.Log(f"Error sending download request: {e}")
...
def sendSearchRequest(file_search, size):
    ...
    try:
        ...
        c.Log(f"Sent search request for '{file_search}' to server.")
    except s.error as e:
...
def connectWithPeer(ip, port_peer):
    ...
    try:
        ...
        c.Log(f"Error connecting to server: {e}")
        return None
...
def calculateStartEndBytes(currIdx, providerCount):
    ...
    if end_byte >= fileSize:
        end_byte = fileSize - 1
    c.Log(f"Current start byte: {start_byte} and current end byte: {end_byte}")
...
def sendRequestedFile(conn, file_requested, start_byte, end_byte):
    try:
        c.Log(f"sendRequestedFile called for {file_requested}, {start_byte}-{end_byte}")
        c.Log(f"Connection info: {conn.getpeername()}")
...
def processDataFromPeer(conn, data):
    if data.startswith(b"START"):
        try:
            ...
            c.Log(f"Error processing command: {e}")
...
def downloadListener(conn, a):
    ...
    while True:
        try:
            ...
            if not data:
                c.Log(f"Peer closed connection: {conn.getpeername() if hasattr(conn, 'getpeername') else conn}")
                with active_file_lock:
            ...
            c.Log(f"Error downloading file: {e}")
            with active_file_lock:
                active_search_request_remaining_parts.pop(conn, None)
                conn.close()
                break
...
def startPeer():
    ...
    c.Log("Server Listener started")

    peer_acceptor_thread = threading.Thread(target=peerAcceptor, daemon=True)
    peer_acceptor_thread.start()
    c.Log("Peer Acceptor started")

    startServing()
    c.Log("START SERVING command has been sent")

    sendFileRepoToServer(files)
    c.Log("START PROVIDING command has been sent")
    time.sleep(5)
    now = time.time()
    startSchedule()
    diff = time.time() - now
